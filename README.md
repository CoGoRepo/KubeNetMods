...
The `lab/` directory contains reproducible local manifests and live-test helpers used while developing KNM. They are tracked in git so behavior can be reproduced, but release archives generated by GitHub Actions package only the compiled `knm` binaries and checksums. GitHub also shows automatic source-code archives for each tag, and those source archives include the repository contents, including `lab/`.
